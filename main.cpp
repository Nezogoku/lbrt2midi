...
#include "pspimg/pspimg.hpp"
#include "printpause.hpp"

#define PROGRAMME_IDENTIFIER "lbrt2midi v5.0"


void printOpt(const char *pName, bool debug) {
...
int main(int argc, char *argv[]) {
    ...
    if (argc < 2) { printOpt(prgm.c_str(), debug); }
    else {
        playmidi player(argc - 1);
        ...
        for (int i = 1; i < argc; ++i) {
            ...
            if (tfle == "-h") { printOpt(prgm.c_str(), debug); break; }
            ...
            if (isLRT || isMID) {
                ...
                if (isMID) player.setSequence(tfle, i - 1);
                else {
                    lbrt pataMID(debug);

                    if (!pataMID.setLRT(tfle)) continue;
                    else if (!play_result && !pataMID.writeMidi()) continue;

                    std::string mid_name = fle.substr(fle.find_last_of("\\/") + 1);
                    unsigned char *mid_data = 0;
                    unsigned mid_size = 0;
                    pataMID.getMidi(mid_size, mid_data);

                    player.setSequence(mid_size, mid_data, mid_name, i - 1);
                }
            }
            else if (isSF2) {
